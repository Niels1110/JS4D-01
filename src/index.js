...
let newActivity = () => {
  ...
  let randomActivityObject =
    activities[Math.floor(Math.random() * activities.length)];

  // Then pick a random item from the list of
  // the object's examples
  let randomExample =
    randomActivityObject.examples[
      Math.floor(Math.random() * randomActivityObject.examples.length)
    ];
...
let newDescription = () => {
  ...
  let description =
    descriptions.descriptions[Math.floor(Math.random() * descriptionsList)];
